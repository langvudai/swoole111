...
class LoadHTML implements HasResponse
{
    ...
    private static $basePath = 'resource/views';
    ...
    {
        $file = sprintf('%s/%s.php', console()->basePath(self::$basePath), preg_replace('/(\.php)$/i', '', $path));
        ...
        $this->path = $file;
        $this->routesRegistered = console(RouteCollector::class, '$routesRegistered');
        $this->std = new Std();
    ...
    private function url($argument): ?string
    {
        ...
        $search = array_search($as, $this->routesRegistered);
