...
final class RouteCollector
{
    ...
    private static $dataGenerator = null;
    private static $dataGeneratorGet = null;
    private static $dataGeneratorPost = null;
    /** @var RouteParser|Std */
    private static $routeParser;
    /** @var array */
    private static $routesRegistered = [];

    /** @var string[] */
    private array $groupAttributeName = ['prefix', 'as', 'middleware'];
    /** @var string[] */
    private array $methodMagic = ['get', 'post', 'put', 'delete', 'patch', 'head', 'trace', 'connect', 'options'];
    /** @var string */
    ...
    {
        if (isset(self::$routesRegistered[$method.$uri])) {
        ...
        self::$routesRegistered[$method.$uri] = $as;
    }
     ...
     * @param $action
     */
    private static function dataGenerator(string $httpMethod, string $route, $action)
    {
        $arrRouteData = self::$routeParser->parse($route);

        if (preg_match('/^(get|delete)$/i', $httpMethod)) {
            foreach ($arrRouteData as $routeData) {
                self::$dataGeneratorGet->addRoute($httpMethod, $routeData, $action);
            }
        } elseif (preg_match('/^(post|put)$/i', $httpMethod)) {
            foreach ($arrRouteData as $routeData) {
                self::$dataGeneratorPost->addRoute($httpMethod, $routeData, $action);
            }
        } else {
            foreach ($arrRouteData as $routeData) {
                self::$dataGenerator->addRoute($httpMethod, $routeData, $action);
            }
        }
    }
    ...
    public final function __construct(string $prefix = null)
    {
        ...
        if (!self::$dataGenerator) {
            self::$dataGenerator = new GroupCountBased();
            self::$dataGeneratorGet = new GroupCountBased();
            self::$dataGeneratorPost = new GroupCountBased();
            self::$routeParser = new Std();
        }
    }
    ...
    {
        if (in_array($name, $this->methodMagic, true) && 3 === count($args)) {
            $this->addRoute(strtoupper($name), $args[0], $args[1], $args[2]);
        } elseif (in_array($name, $this->methodMagic, true) && 2 === count($args)) {
    ...
    private function mergeGroup(array $new, array $old) :array
    {
        ...
        // filter
        $arrExcept = array_diff_key($old, array_flip(['prefix', 'as']));

        return array_merge_recursive($arrExcept, $new);
    }
